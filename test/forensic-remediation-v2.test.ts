...
test('REMEDIATION [BUG-003]: AI routes use canonical Google Gemini model IDs', () => {
  ...
  assert.match(chatCode, /gemini-2\.5-flash/, 'Chat route must use gemini-2.5-flash');
  assert.doesNotMatch(chatCode, /gemini-3\.5-flash-lite/, 'Chat route must NOT use nonexistent gemini-3.5-flash-lite');

  assert.match(categorizeCode, /gemini-2\.5-flash/, 'Categorize route must use gemini-2.5-flash');
  assert.doesNotMatch(categorizeCode, /gemini-3\.6-flash/, 'Categorize route must NOT use nonexistent gemini-3.6-flash');

  assert.match(insightsCode, /gemini-2\.5-flash/, 'Insights route must use gemini-2.5-flash');
  assert.doesNotMatch(insightsCode, /gemini-3\.6-flash/, 'Insights route must NOT use nonexistent gemini-3.6-flash');
});
