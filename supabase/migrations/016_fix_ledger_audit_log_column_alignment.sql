-- ==============================================================================
-- NISFLOW FINANCE — MIGRATION 016: LEDGER AUDIT LOG COLUMN SCHEMA ALIGNMENT
-- ==============================================================================
--
-- Incident Remediation:
-- Replaces stale/drifted column references in post_journal_entry
-- and post_reversal_entry with the canonical column `action` on `public.ledger_audit_log`.
--
-- Invariants Preserved:
-- 1. Double-Entry Balancing Invariant (SUM(Debits) === SUM(Credits))
-- 2. Caller Authorization & Actor Derivation (Strict auth.uid() scoping)
-- 3. Atomic Account Balance Dual-Write (balance & current_balance synchronized)
-- 4. Cryptographic SHA-256 Checksum Hashing (pgcrypto)
-- 5. Row-Level Security & Multi-Tenant Isolation
-- ==============================================================================

-- 1. Ensure pgcrypto extension is enabled for cryptographic SHA-256 hashing
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- 2. Hardened post_journal_entry with canonical ledger_audit_log 'action' column
...
BEGIN
    ...
    -- 9. Insert Immutable Audit Log Record with authoritative actor_id and canonical 'action' column
...
-- 3. Hardened post_reversal_entry with canonical ledger_audit_log 'action' column
