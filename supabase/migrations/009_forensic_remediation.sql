-- ==============================================================================
-- NISFLOW FINANCE — MIGRATION 009: PRE-LAUNCH FORENSIC SECURITY & INTEGRITY REMEDIATION
-- ==============================================================================
--
-- Fixes:
-- 1. True SHA-256 Cryptographic Hashing for Ledger Audit Trail (pgcrypto)
-- 2. Account Cached Balance Synchronization (balance <-> current_balance parity)
-- 3. audit_logs Row-Level Security Hardening (Remove public insert bypass)
-- 4. Entity Type Differentiation & Foreign Key Hardening
-- ==============================================================================

-- 1. Enable pgcrypto for cryptographic SHA-256 hashing
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- 2. Ensure accounts table has synchronized balance columns
...
-- Ensure loans table supports soft deletion / archival
ALTER TABLE public.loans ADD COLUMN IF NOT EXISTS is_deleted BOOLEAN DEFAULT FALSE;

-- 3. Update post_journal_entry Stored Procedure with SHA-256 Hashing & Account Parity
...
-- 4. Update post_reversal_entry Stored Procedure with SHA-256 Hashing
...
-- 5. Update get_ledger_account_balance & reconcile_ledger_balances for mathematical reversal consistency
...
BEGIN
    ...
    IF v_account_type IN ('asset', 'expense') THEN
        -- Normal Debit balance: Balance = Debits - Credits across all posted & reversed entries (reversals contain offsetting lines)
    ...
    ELSE
        -- Normal Credit balance: Balance = Credits - Debits across all posted & reversed entries
...
-- 6. Hardening audit_logs Row-Level Security: Eliminate unrestricted INSERT policy
